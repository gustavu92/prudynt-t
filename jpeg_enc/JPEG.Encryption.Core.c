...
int jpeg_process_buffer(
    ...
    int keyLen = 0;
    while(password[keyLen] != '\0' && keyLen < AES_KEY_SIZE){
        key[keyLen] = password[keyLen];
        keyLen++;
    }
    ...
    if(cryptoModeChar == 'e')
        cryptoMode = ENCRYPT;
    else if(cryptoModeChar == 'd')
        cryptoMode = DECRYPT;
    else
        return 1;

    *sizeOut = sizeIn * 2;
    *bufferOut = (unsigned char*)calloc(*sizeOut, sizeof(unsigned char));
    if(!*bufferOut) return 1;

    initAesPrng(key, AES_KEY_SIZE);

    int *roiArrayCopy = NULL;
    if(roiArraySize > 0){
        roiArrayCopy = (int*)malloc(sizeof(int) * roiArraySize);
        if(!roiArrayCopy){
            free(*bufferOut);
            return 1;
        }
        for(int i = 0; i < roiArraySize; i++)
            roiArrayCopy[i] = roiArray[i];
    }

    int result = jpegCrypto(
        bufferIn,
        *bufferOut,
        (long)sizeIn,
        (long*)sizeOut,
    ...
    if(roiArrayCopy) free(roiArrayCopy);
