...
static uint32_t dc_prng_state = 0;
...
// 	return retval;
// }
uint32_t dc_GetRandomNumber(uint8_t n_bits) {
    // LCG: Linear Congruential Generator
    dc_prng_state = (1103515245 * dc_prng_state + 12345) & 0x7FFFFFFF;
    return dc_prng_state & ((1U << n_bits) - 1);
}
...
void initAesPrng(unsigned char* keyArray, int keyArraySize){
	...
	// --- DEBUG: imprime a chave ---
    printf("DEBUG: tempKey = ");
    for(i = 0; i < 16; i++)
